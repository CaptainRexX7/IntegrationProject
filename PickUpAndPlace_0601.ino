...
void loop() {
  ...
  switch (state){
    ...
    case FollowLine:
      ...
      followLine(turnTime);
      if(LeftSensor() == 2 && LeftSensor() == 2){ //when these conditions are met it will be at the green line 
        Stop();
        if(firstTime){
          firstTime = 0;
          state = End; //PlaceBox;
...
void followLine(int time) 
{
  ...
  else if(isColorL == 1){ //if the left sensor detects the line color but the right does not
    RotateLft();
    delay(time);
    firstTurn = 0;
    Stop();
