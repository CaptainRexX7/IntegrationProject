...
#define targetDistance 13 // the vehicle needs to be within 10cm
...
//Timing
int speedR = 25; //slowed down to give the light sensors more time to read
int speedL = 25; //slowed down to give the light sensors more time to read
...
#define straight 7900
#define PUStraight 2000
#define PBStraight 2500
#define Nine 4500
#define oneeighty 8500
...
enum {FindBox, FollowLine, PlaceBox, End, Return}; //I removed the return state
...
void setup() {
  ...
  state = FollowLine;//FindBox;//FollowLine;//PlaceBox;// FindBox;//FollowLine;//FindBox;//PlaceBox;
  ...
  if(Ultrasonic() <= targetDistance){
    ...
    if(analogRead(forceSensor) < forceValue){ //if there is no force onto the side then it is not a large box and it will close more to be able to pick up the smal box
      ...
      if(isRed){ //this is sensing for red if not red then it is blue 
        ...
        delay(turnTime1*.74);
        Forward();
        delay(straight*.75);//straightline
        RotateRt();
        delay(turnTime1*.74);
        Stop();
        delay(1000);
    ...
    else{ // for the large box
      isLarge = 1; //documenting that the box is large
      MoveArm(RestingArm);// moves arm so that it is out of the way of the ultrasonic sensor
      ArmSensor(); //checks the color of the box //the light sensor requires at least 250ms to read
      delay(2000);
      Backward();
...
{ 
  ...
  //  Stop();
  //  delay(500);
  //  if(LeftSensor() == 2 && RightSensor() == 0){
  //    FLLRotateLft();
  //  }
  //  else if(LeftSensor() == 0 && RightSensor() == 2){
  //    FLLRotateRt();
    //Forward();
    //delay(500);
    //Stop();
  ...
  // }
  //}
  if(LeftSensor() == 1){ //else if(LeftSensor() == 1){
    //Stop();
    //delay(250);
    Backward();
    delay(450);
    FLRotateLft(); //FLRotateLft();
    delay(650);
    Forward();
    delay(300);
  }
...
void placeBox(){
  if(isLarge == 0){
    ...
    delay(Nine*.9);
    Forward();
   // delay(PBStraight*3.95);
   if(isColorR == 2 || isColorL == 2){
    Stop();
    state = Return;
   }
  }
...
void FLRotateRt() {
  analogWrite(ENA, PWMvalR*3);
  analogWrite(ENB, PWMvalL*3);
...
void FLLRotateRt() {
  analogWrite(ENA, PWMvalR*5);
  analogWrite(ENB, PWMvalL*5);
