...
{
  if(LeftSensor() == 1){ //if the left sensor detects the line color but the right does not
    //RotateLft();
    //delay(time);
    //firstTurn = 0;
    Stop();
  } 
  else if(RightSensor() == 1){ //if the right sensor detects the line color but the left does not
    //RotateRt();
    //delay(time);
    //firstTurn = 0;
    Stop();
